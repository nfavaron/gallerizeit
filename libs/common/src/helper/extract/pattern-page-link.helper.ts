 ...
 */
export function extractPatternPageLinkHelper(links: ImageLinkModel[], imageLinkPattern: string): string {
    ...
    explode.forEach(part => {
      ...
      if (part) {
        ...
        if (p.indexOf('@page@') === -1) {
          ...
          testPattern = regExpEscapeHelper(testPattern, '/');
        ...
        if (numbers.length > 1) {
          ...
          // Extract number
          let testPattern = regExpEscapeHelper(p + part, '/');
          testPattern = testPattern.replace(/@page@/gi, '[0-9]+');
